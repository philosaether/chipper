...
- **Type-checking helpers for predicates**: `!isNaN(Number(ctx.cadenceType))` works but is noisy. Want a helper like `isNumeric(value)` or `isString(value)` for use in `present` predicates and `contingentOn` configs.
- **Punctuation method on clause builder**: `.punctuation(',')` or similar that renders a comma, period, etc. based on whether the parent clause is the last active clause. Solves trailing punctuation in multi-clause sentences.
